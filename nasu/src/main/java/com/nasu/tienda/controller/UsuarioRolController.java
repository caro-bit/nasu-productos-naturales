...
public class UsuarioRolController {
    ...
    public String listado(Model model) {
        ...
        Map<Integer, List<String>> rolesPorUsuario = new HashMap<>();
        for (Usuario u : usuarios) {
            rolesPorUsuario.put(u.getIdUsuario(), usuarioService.getRoles(u.getIdUsuario()));
        }

        model.addAttribute("usuarios", usuarios);
        model.addAttribute("rolesPorUsuario", rolesPorUsuario);
        model.addAttribute("usuario", new Usuario()); // necesario para que el fragmento "asignar" no falle al parsear el archivo completo
